...
[cite_start]The default algorithm in Java 7+ and Python[cite: 1586]. It uses **two pivots** ($p, q$ with $p < q$) to partition the array into three regions:
1.  $x < p$
2.  $p \le x \le q$
3.  $x > q$
...
* [cite_start]**Reason:** It minimizes **Branch Mispredictions** [cite: 1583-1585]. Modern CPUs pipeline instructions; a random conditional jump (like in Quicksort partition) clears the pipeline. Dual-Pivot reduces the number of memory accesses and behaves better with CPU caches, making the "cost per comparison" lower.

### 1.3 Bounded Quicksort
Standard Quicksort is recursive. [cite_start]In the worst case (unbalanced partitions), the recursion depth can reach $O(N)$, causing a **Stack Overflow**[cite: 1614].

**The Solution:**
[cite_start]To guarantee $O(\log N)$ stack space, we rely on **Tail Recursion Elimination** on the larger sub-problem[cite: 1616].
