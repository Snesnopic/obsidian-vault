...
$$
2. **Step:** A path of length $L$ implies a path of length $L-1$ to some node $z$, and an edge from $z$ to $j$. $$ P(\text{path } L) \le \sum_{z} P(\text{path } i \to z \text{ len } L-1) \cdot P(\text{edge } z \to j) $$ $$ \le m \cdot \left( \frac{1}{m c^{L-1}} \right) \cdot \frac{1}{cm} = \frac{1}{m c^L} $$ **Conclusion:** The probability of a path decreases exponentially with length. The probability of a cycle (path from $i$ to $i$) is very low, and a double cycle ("Bicycle") is $O(1/N^2)$.