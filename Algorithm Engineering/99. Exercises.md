...
**Problem:** 1. Build a **Min-Treap** (inserting node by node) with the following pairs of $\langle \text{Key}, \text{Priority} \rangle$:
...
* **Insert (M, 3):** $M > L$ (Right). Priority $3 < 5$ $\to$ Rotate $M$ up. **Root: M**. Left: L.
* **Insert (Z, 10):** $Z > M$. Priority $10 > 3$ $\to$ Right child of M.
* **Insert (D, 2):** $D < M$ (Left). $D < L$ (Left of L). Priority $2 < 5$ and $2 < 3$ $\to$ Rotate $D$ up over $L$, then over $M$. **Root: D**. Right: M.
...
- $F$ has minimal priority, rotates up to the Root.
    
- Left Tree ($T_\le$): D, A.
    
- Right Tree ($T_>$): M, L, Z.
    
    [[#Ex 3.1|Back to Question]]
...
2. **Recurse on $S_<$ (index 0):** `{"abi", "at"}`. Pivot "abi" ('a').
    ...
    - $S_=$: `{"abi", "at"}`. Recurse index **1**.
        
    - Pivot "abi" ($p[1]=\text{'b'}$).
        
    - $S_>$: `{"at"}` ('t' > 'b'). Result: `abi, at`.
...
3. **Recurse on $S_=$ (index 1):** `{"cat", "cast", "car"}`. Pivot "cat" ('a').
    ...
    - Sort `{"car", "cast"}` at index 2 (Pivot "car", 'r' < 's'). Result: `car, cast`.
        
4. Concatenate: abi, at + car, cast, cat.
    
    [[#Ex 4.1|Back to Question]]
...
- `001`: $\gamma$ code. $N=2 \to 11_2 = 3$. Length is 3.
