...
### Ex 3.2: Ternary Search Tree Insertion 
**Problem:** Insert the following set of strings into an initially empty Ternary Search Tree (TST):
$$S = \{\text{"cat", "abi", "cast", "car", "at"}\}$$
**Rules:**
* Each node contains a split character `char` and a value `i` (index of the character being checked).
* Use the standard TST structure: Left ($< c$), Eq ($= c$, advances $i$), Right ($> c$).
* When a collision occurs in an "Eq" child, split the node.
[[#Sol 3.2 (TST)|See Solution]]
...
### Sol 3.2 (TST)
**Sequence:** `cat, abi, cast, car, at`.

1.  **Insert "cat":**
    * Root node `(C, i=0)`.
    * Middle child (Eq) $\to$ "cat" (or pointer to rest).
2.  **Insert "abi":**
    * Compare 'a' with 'c'. 'a' < 'c'.
    * Go **Left**. Empty $\to$ Create node `(A, i=0)`.
    * Middle child $\to$ "abi".
3.  **Insert "cast":**
    * Compare 'c' with 'c'. Match. Go **Middle**.
    * Collision with "cat". Split!
    * Current `i=0`. Next char for "cat" is 'a', for "cast" is 'a'. Match.
    * Create node `(A, i=1)`. Middle child holds both.
    * Split again. Next char "cat"='t', "cast"='s'. 's' < 't'.
    * Create node `(T, i=2)`.
    * Left child $\to$ "cast" (ends at 's'?). Wait, "cast" continues. Node should be `(S, i=2)`.
    * Let's trace rigorously:
        * `(C, 0)` Eq $\to$ `(A, 1)` Eq $\to$ `(T, 2)` (from "cat").
        * Insert "cast": `(C,0)` match $\to$ `(A,1)` match $\to$ compare 's' vs 't'. 's' < 't'.
        * `(T, 2)` Left $\to$ Create `(S, 2)`.
        * `(S, 2)` Eq $\to$ "cast" (rest "t").
4.  **Insert "car":**
    * `(C,0)` match $\to$ `(A,1)` match $\to$ `(T,2)` compare 'r' vs 't'. 'r' < 't'.
    * Go Left $\to$ `(S,2)`. Compare 'r' vs 's'. 'r' < 's'.
    * Go Left of `(S,2)` $\to$ Create `(R, 2)`. Eq $\to$ "car".
5.  **Insert "at":**
    * Compare 'a' vs 'c'. Go Left $\to$ `(A,0)`.
    * Match 'a'. Go Eq $\to$ Collision with "abi".
    * Split "abi" at $i=1$. 'b' vs 't'. 'b' < 't'.
    * Create node `(B, 1)`. Eq $\to$ "abi".
    * "at" has 't' > 'b'. Right of `(B,1)` $\to$ Create `(T, 1)`.
    * `(T,1)` Eq $\to$ "at".
    ...
    * Eq: `(A,1)`
        * Eq: `(T,2)`
            ...
            * Left: `(S,2)`
                * Eq: "cast"
                * Left: `(R,2)` $\to$ "car"
[[#Ex 3.2: Ternary Search Tree Insertion|Back to Question]]
