...
## Part 6: Advanced Concurrency (Weak Semantics & Logics)

### Questions
56. **Weak Transition:** What is a Weak Transition $P \stackrel{\alpha}{\Rightarrow} Q$? [[#A6.1|Answer]]
57. **Weak Bisimulation:** What is the definition of Weak Bisimulation? [[#A6.2|Answer]]
58. **Congruence Issue:** Why is Weak Bisimilarity ($\approx$) NOT a congruence? (The Counterexample) [[#A6.3|Answer]]
59. **Observational Congruence:** What is Weak Observational Congruence? [[#A6.4|Answer]]
60. **HML:** What is the syntax and semantics of Hennessy-Milner Logic? [[#A6.5|Answer]]
61. **HM Theorem:** What is the relation between HML equivalence and Strong Bisimilarity? [[#A6.6|Answer]]
62. **Temporal Logics:** What is the difference between Linear Time (LTL) and Branching Time (CTL)? [[#A6.7|Answer]]
63. **Mu-Calculus:** What is the fixpoint operator $\mu X.\phi$ in Mu-Calculus? [[#A6.8|Answer]]
...
### A6.1
**Weak Transition ($\stackrel{\alpha}{\Rightarrow}$)**
Abstracts away from internal $\tau$ actions.
* **Silent weak transition ($\stackrel{\tau}{\Rightarrow}$):** $P \xrightarrow{\tau}^* P$ (Zero or more $\tau$ steps).
* **Observable weak transition ($\stackrel{\alpha}{\Rightarrow}$):** $P \xrightarrow{\tau}^* \xrightarrow{\alpha} \xrightarrow{\tau}^* P'$ (Internal steps before and after the visible action).
* *Note:* $\stackrel{\tau}{\Rightarrow}$ is reflexive (can do nothing), while $\xrightarrow{\tau}$ is not.
[[#Part 6: Advanced Concurrency (Weak Semantics & Logics)|Back to Q]]

### A6.2
**Weak Bisimulation ($\approx$)**
A relation $R$ is a weak bisimulation if for all $(P,Q) \in R$:
1. If $P \xrightarrow{\alpha} P'$, then $\exists Q'$ such that $Q \stackrel{\alpha}{\Rightarrow} Q'$ and $(P', Q') \in R$.
2. If $P \xrightarrow{\tau} P'$, then $\exists Q'$ such that $Q \stackrel{\tau}{\Rightarrow} Q'$ (which allows $Q'=Q$) and $(P', Q') \in R$.
*Key Difference:* A strong move $\xrightarrow{\alpha}$ is matched by a weak move $\stackrel{\alpha}{\Rightarrow}$.
[[#Part 6: Advanced Concurrency (Weak Semantics & Logics)|Back to Q]]

### A6.3
**Congruence Issue (Counterexample)**
Weak bisimilarity is **not** a congruence with respect to Choice ($+$).
* Let $P = \alpha.Q$ and $Q = \tau.\alpha.Q$. We have $P \approx Q$.
* Context $C[\cdot] = \cdot + \beta.\textbf{nil}$.
* $P + \beta.\textbf{nil} \not\approx \tau.\alpha.Q + \beta.\textbf{nil}$.
    * *Reason:* The second process can do $\tau$ to reach $\alpha.Q$ (discarding the $\beta$ option). The first process cannot simulate this "silent commitment" because any $\tau$-move from $P+\beta.\textbf{nil}$ must be matched by a weak $\tau$-move, but $P+\beta.\textbf{nil}$ has no $\tau$ transitions.
[[#Part 6: Advanced Concurrency (Weak Semantics & Logics)|Back to Q]]

### A6.4
**Weak Observational Congruence**
The largest congruence contained in weak bisimilarity.
Two processes are observationally congruent if:
1. They are weakly bisimilar.
2. Every *initial* $\tau$ move of one must be matched by at least one $\tau$ move of the other (cannot stay idle).
*Slogan:* "First step strong, subsequent steps weak."
[[#Part 6: Advanced Concurrency (Weak Semantics & Logics)|Back to Q]]

### A6.5
**Hennessy-Milner Logic (HML)**
A modal logic to describe local properties of processes.
* **Syntax:** $F ::= \text{tt} \mid \text{ff} \mid F_1 \land F_2 \mid F_1 \lor F_2 \mid \langle \alpha \rangle F \mid [\alpha]F$.
* **Semantics:**
    * $P \models \langle \alpha \rangle F$: $\exists P'$ s.t. $P \xrightarrow{\alpha} P'$ and $P' \models F$ (Possibility).
    * $P \models [\alpha]F$: $\forall P'$ s.t. $P \xrightarrow{\alpha} P'$, $P' \models F$ (Necessity / After).
[[#Part 6: Advanced Concurrency (Weak Semantics & Logics)|Back to Q]]

### A6.6
**HM Theorem**
For **finite-branching** processes (image finite LTS):
$$P \sim Q \iff (\forall F \in \text{HML}. \ P \models F \iff Q \models F)$$
*Significance:* Logical equivalence characterizes strong bisimilarity. If two processes are not bisimilar, there exists an HML formula distinguishing them.
[[#Part 6: Advanced Concurrency (Weak Semantics & Logics)|Back to Q]]

### A6.7
**LTL vs CTL**
* **LTL (Linear Temporal Logic):** Formulas are interpreted over **paths** (sequences of states). Models time as a line.
    * Quantifiers: $X$ (Next), $F$ (Finally), $G$ (Globally), $U$ (Until).
    * Implicit universal quantification over all paths starting from $s_0$.
* **CTL (Computation Tree Logic):** Formulas are interpreted over the **branching tree** of executions.
    * Explicit path quantifiers: $A$ (For All paths), $E$ (Exists a path).
    * *Difference:* $AG(EF \phi)$ ("Always possible to recover") is in CTL but cannot be expressed in LTL.
[[#Part 6: Advanced Concurrency (Weak Semantics & Logics)|Back to Q]]

### A6.8
**Mu-Calculus Fixpoints**
The Mu-Calculus adds recursive definitions to HML.
* $\mu X.\phi$: **Least Fixpoint**. Represents finite loopings or "eventualities" (something good *must* happen eventually).
    * Example: $\mu X. (\phi \lor \langle \cdot \rangle X)$ (Reachability).
* $\nu X.\phi$: **Greatest Fixpoint**. Represents infinite loopings or "invariants" (something good *always* holds).
    * Example: $\nu X. (\phi \land [\cdot]X)$ (Safety / Invariance).
[[#Part 6: Advanced Concurrency (Weak Semantics & Logics)|Back to Q]]

---

## Part 7: KAT (Kleene Algebra with Tests)

### Questions
64. **Kleene Algebra:** What is a Kleene Algebra (KA)? [[#A7.1|Answer]]
65. **Tests:** What is a Test in the context of KA? [[#A7.2|Answer]]
66. **KAT Definition:** What is a Kleene Algebra with Tests (KAT)? [[#A7.3|Answer]]
67. **Modeling IMP:** How are IMP commands encoded into KAT? [[#A7.4|Answer]]
68. **While Loop:** What is the KAT expression for `while b do p`? [[#A7.5|Answer]]
69. **Hoare Logic:** How is the Hoare Triple $\{P\}c\{Q\}$ encoded in KAT? [[#A7.6|Answer]]
...
### A7.1
**Kleene Algebra (KA)**
A structure $(K, +, \cdot, *, 0, 1)$ that is:
1. An **Idempotent Semiring** under $(+, \cdot, 0, 1)$:
   * $a + a = a$ (Idempotence).
   * Distributivity of $\cdot$ over $+$.
2. Equipped with a **Kleene Star** $(*)$ satisfying unfolding and induction axioms:
   * $1 + aa^* \le a^*$ (Unfolding).
   * $1 + a^*a \le a^*$ (Unfolding).
   * $ab \le b \implies a^*b \le b$ (Induction).
[[#Part 7: KAT (Kleene Algebra with Tests)|Back to Q]]

### A7.2
**Tests (Boolean Algebra)**
A **test** is an element of a Boolean Algebra $B$ embedded in $K$ ($B \subseteq K$).
* The meet is logical AND (multiplication $\cdot$).
* The join is logical OR (addition $+$).
* Every test $b \in B$ has a complement $\bar{b}$ such that $b\bar{b} = 0$ and $b+\bar{b} = 1$.
* *Intuition:* Tests behave like "filters". If the test is true, proceed (identity); if false, block (zero).
[[#Part 7: KAT (Kleene Algebra with Tests)|Back to Q]]

### A7.3
**KAT Definition**
A **KAT** is a two-sorted structure $(K, B)$ where:
1. $K$ is a Kleene Algebra.
2. $B \subseteq K$ is a Boolean Algebra.
3. $(B, +, \cdot, 0, 1)$ is a subalgebra of $K$.
*Usage:* $K$ models actions/programs, $B$ models assertions/guards.
[[#Part 7: KAT (Kleene Algebra with Tests)|Back to Q]]

### A7.4
**Encoding IMP in KAT**
We use a translation function $k : \text{IMP} \to \text{KAT}$:
1. **Action:** $k(x := a)$ is treated as an atomic element $a \in K$.
2. **Sequence:** $k(c_1 ; c_2) = k(c_1) \cdot k(c_2)$.
3. **Choice:** $k(\textbf{if } b \textbf{ then } p \textbf{ else } q) = b \cdot k(p) + \bar{b} \cdot k(q)$.
   * *Logic:* If $b$ is true, execute $p$; if $b$ is false ($\bar{b}$), execute $q$. The sum represents the union of these disjoint possibilities.
[[#Part 7: KAT (Kleene Algebra with Tests)|Back to Q]]

### A7.5
**While Loop Encoding**
$$k(\textbf{while } b \textbf{ do } p) = (b \cdot k(p))^* \cdot \bar{b}$$
*Explanation:*
* $(b \cdot k(p))^*$: Repeat "check $b$ then execute $p$" zero or more times.
* $\cdot \bar{b}$: Finally, verify that $b$ is false (exit condition).
* *Note:* Without the final $\bar{b}$, the loop could stop arbitrarily even if $b$ is true.
[[#Part 7: KAT (Kleene Algebra with Tests)|Back to Q]]

### A7.6
**Hoare Logic in KAT**
A partial correctness triple $\{P\}c\{Q\}$ ("If start with $P$ and $c$ terminates, then $Q$ holds") is encoded as an equation in KAT:
$$P \cdot c \cdot \bar{Q} = 0$$
*Meaning:* It is impossible ($=0$) to satisfy start condition $P$, execute $c$, and end up in a state satisfying $\neg Q$.
*Alternative form:* $P \cdot c \le c \cdot Q$ (Pushing the property through).
[[#Part 7: KAT (Kleene Algebra with Tests)|Back to Q]]