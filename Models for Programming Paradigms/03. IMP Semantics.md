...
* **Skip:** $k(\textbf{skip}) = 1$ (Identity, no op).
* **Assignment:** $k(x:=e)$ is an atomic action (relation update).
...
The standard interpretation of KAT for imperative programs is the **Relational Model**:
* **Carrier:** $\mathcal{P}(\Sigma \times \Sigma)$ (Relations on states).
* **Multiplication ($\cdot$):** Relational composition ($R ; S$).
* **Addition ($+$):** Union ($R \cup S$).
* **Tests ($b$):** Coreflexive relations (subsets of the identity).
    * $\sem{b} = \{(\sigma, \sigma) \mid \sigma \models b\}$.

### 8.3 Algebraic Verification
Using KAT axioms (e.g., $(ab)^* a = a (ba)^*$), we can prove program equivalences purely algebraically, without induction on semantics.
**Example:** Loop Unrolling
$$
(b \cdot c)^* \cdot \neg b \equiv \neg b + b \cdot c \cdot (b \cdot c)^* \cdot \neg b
$$
Corresponds to `while b do c` $\equiv$ `if b then (c; while b do c) else skip`.