...
- [[#Rank and Select]]
 - Problem definition
 - Naive Solution vs Succint Requirements
 - Jacobson's Solution: Superblocks, Blocks, Popcount ($n + o(n)$ bits) 
 - Compressed Solution: Rank/Select via Elias-Fano
...
$$suff_i = c \cdot \alpha \dots$$ $$suff_j = c \cdot \alpha \dots$$
3.  Now consider $suff_{i+1}$ and $suff_{j+1}$. They are obtained by dropping the first char $c$.
$$suff_{i+1} = \alpha \dots$$ $$suff_{j+1} = \alpha \dots$$
...
- **Implicit Encoding (Zero bits):** If the sequence is dense (e.g., $10, 11, 12$), the calculated range $R$ for the middle element will have size 1. The log of 1 is 0. The algorithm emits **0 bits**, implicitly encoding the numbers purely by their constraints.


### 6.8 Elias-Fano Encoding

A specialized encoding for **monotone increasing sequences** (sorted integers) that allows **constant-time access** ($O(1)$) to the $i$-th element, making it extremely powerful for search engines and inverted indexes.

**Algorithm:**

Given a sorted sequence $S = s_1, \dots, s_n$ with max value $u = s_n + 1$:

1. **Parameters:**
    
    - Low-bits length: $\ell = \lfloor \log_2(u/n) \rfloor$.
        
    - High-bits length: $h = \lceil \log_2 u \rceil - \ell$.
        
2. **Decomposition:** Each number $s_i$ is split into a **Low Part** (last $\ell$ bits) and a **High Part** (remaining upper bits).
    
    - $L[i] = s_i \pmod{2^\ell}$.
        
    - $H[i] = \lfloor s_i / 2^\ell \rfloor$.
        
3. **Storage:**
    
    - **Array L:** Stores the low bits of all numbers explicitly ($n \times \ell$ bits).
        
    - **Array H:** Stores the high parts using a **Unary-like** encoding. We construct a bitvector where we write $k$ zeros followed by a 1, where $k$ is the number of items having the specific high value $j$.
        
    - Conceptually: It is a bitvector of length $2n + n \lceil \log(u/n) \rceil$ bits.
        

**Complexity & Space:**

- **Space:** $2n + n \log(u/n)$ bits. This is very close to the information-theoretic lower bound (entropy of the gap distribution).
    
- **Access(i):** To retrieve $S[i]$, we combine:
    
    - Low part: Read $\ell$ bits from $L$ at offset $i \cdot \ell$.
        
    - High part: Use **Select_1(i)** on the $H$ array to find the position of the $i$-th 1. The number of zeros before it determines the high value.
        
    - $S[i] = \text{High} \cdot 2^\ell + \text{Low}$.
        

**Operations:**

- **Access($i$):** Returns $S[i]$ in $O(1)$ time.
    
- **NextGEQ($x$):** Returns the smallest $S[i] \ge x$. Uses `Select_0` on the high part to jump directly to the bucket where $x$ might be, avoiding full decompression.
